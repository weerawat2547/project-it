...
import { Wrench, AlertCircle, CheckCircle, Eye, EyeOff, ArrowRight, Sparkles, CheckCircle2, User, KeyRound, Mail, Phone, Building2 } from 'lucide-react';
...
export default function Register() {
  ...
  const [isExiting, setIsExiting] = useState(false); // State สำหรับควบคุม Exit Animation
  ...
  // ฟังก์ชันสำหรับสลับกลับไปหน้า Login พร้อม Animation
  const handleGoToLogin = (e: React.MouseEvent) => {
    e.preventDefault();
    setIsExiting(true);
    setTimeout(() => {
      navigate('/');
    }, 300); // รอ animation เล่นเสร็จ 300ms แล้วค่อยเปลี่ยนหน้า
  };
          ...
          <div className="bg-slate-900/80 rounded-3xl shadow-2xl shadow-black/90 p-8 sm:p-10 border border-slate-800/90 backdrop-blur-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                />
                <Field
                  id="department" label="คณะ/หน่วยงาน" icon={Building2}
